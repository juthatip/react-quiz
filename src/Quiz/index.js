  ...
  return (
    <>
      {quiz && <div>
        ...
        <h2>{quiz.question}</h2>
        {Object.keys(quiz.answers).map((d, i) => <div key={i} onClick={() => selectAnswer(quiz.id, quiz.answers[d], i)} className={isSelect === quiz.answers[d] ? 'select' : ''}>{d}.{quiz.answers[d]}</div>)}
        {currentQuestion > 1 && <button onClick={setPrevQuestion}>prev</button>}
        {currentQuestion === allQuestion.length ? <button onClick={finishQuiz} disabled={!isSelect ? true : false}>finish</button> : <button onClick={setNextQuestion} disabled={!isSelect ? true : false}>next</button>}
      </div>
      }
    </>
  )
